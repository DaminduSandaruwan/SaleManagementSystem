...
Public Class frmCustomer
    ...
    Private Sub SetupDGVData()
        With dgvData
            ...
            .Columns.Add("NIC", "NIC")
            .Columns.Add("TitleName", "Title")
            .Columns.Add("CustomerName", "Region")
            .Columns.Add("RegionName", "Customer Name")
            .Columns.Add("Email", "Email")
            .Columns.Add("Mobile", "Mobile")
            .Columns.Add("LineID", "Address")
            .Columns.Add("Home", "Home")
    ...
    Private Sub RetrieveData(Optional ByVal blnSearch As Boolean = False)
        strSQL = " SELECT tblCustomer.CustomerPK, tblCustomer.CustomerID, tblCustomer.NIC, tblCustomer.CustomerName, " &
                    " tblCustomer.Email, tblCustomer.Mobile, tblCustomer.LineID, tblCustomer.Home, tblCustomer.PictureName, tblCustomer.Note, " &
                    " tblTitle.TitleName, tblRegion.RegionName " &
                    " FROM [tblTitle] INNER JOIN (tblRegion INNER JOIN tblCustomer ON " &
                    " tblRegion.RegionPK = tblCustomer.RegionFK) ON tblTitle.TitlePK = tblCustomer.TitleFK "

        '// blnSearch = True for Serach
        If blnSearch Then
            strSQL = strSQL &
                " WHERE " &
                " [CustomerID] " & " Like '%" & txtSearch.Text & "%'" & " OR " &
                " [NIC] " & " Like '%" & txtSearch.Text & "%'" & " OR " &
                " [TitleName] " & " Like '%" & txtSearch.Text & "%'" & " OR " &
                " [RegionName] " & " Like '%" & txtSearch.Text & "%'" & " OR " &
        ...
        Try
            ...
            While DR.Read
                With dgvData
                    ...
                    .Rows(i).Cells(1).Value = DR.Item("CustomerID").ToString
                    .Rows(i).Cells(2).Value = DR.Item("NIC").ToString
                    .Rows(i).Cells(3).Value = DR.Item("TitleName").ToString
                    .Rows(i).Cells(4).Value = DR.Item("RegionName").ToString
                    .Rows(i).Cells(5).Value = DR.Item("CustomerName").ToString
                    .Rows(i).Cells(6).Value = DR.Item("Email").ToString
                    .Rows(i).Cells(7).Value = DR.Item("Mobile").ToString
                    .Rows(i).Cells(8).Value = DR.Item("LineID").ToString
                    .Rows(i).Cells(9).Value = DR.Item("Home").ToString
    ...
    Private Sub dgvData_DoubleClick(sender As Object, e As System.EventArgs) Handles dgvData.DoubleClick
        ...
        Call PopulateComboBox(cmbTitle, "tblTitle", "TitleName", "TitlePK")
        cmbTitle.Text = "" & dgvData.Item(3, iRow).Value
        Call PopulateComboBox(cmbRegion, "tblRegion", "RegionName", "RegionPK")
        cmbRegion.Text = "" & dgvData.Item(4, iRow).Value
        '//
        txtCustomerName.Text = "" & dgvData.Item(5, iRow).Value
        txtEmail.Text = "" & dgvData.Item(6, iRow).Value
        txtMobile.Text = "" & dgvData.Item(7, iRow).Value
        txtYear.Text = "" & dgvData.Item(8, iRow).Value
        txtHome.Text = "" & dgvData.Item(9, iRow).Value
    ...
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        ...
        If NewData Then
            ...
            strSQL =
                " SELECT Count(tblCustomer.CustomerID) AS CountCustomerID FROM tblCustomer " &
                " WHERE CustomerID = " & "'" & txtFullname.Text & "'"
            If DuplicateName(strSQL) Then
                MessageBox.Show("Duplicate CustomerID, please enter new value.", "Report Status",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txtFullname.Focus()
                Exit Sub
            End If
            ...
            strSQL = "INSERT INTO tblCustomer(" &
                " CustomerPK, CustomerID, NIC, TitleFK, RegionFK, CustomerName, Email, Mobile, LineID, Home, " &
                " PictureName, [Note], " &
                ...
                "'" & txtFullname.Text & "'," &
                "'" & txtNIC.Text & "'," &
                "'" & PKComboBox(cmbTitle, "tblTitle", "TitleName", "TitlePK") & "'," &
                "'" & PKComboBox(cmbRegion, "tblRegion", "RegionName", "RegionPK") & "'," &
        ...
        Else
            ...
            strSQL = "UPDATE tblCustomer SET " &
                " [CustomerID]='" & txtFullname.Text & "', " &
                " [NIC]='" & txtNIC.Text & "', " &
                " [TitleFK]=" & PKComboBox(cmbTitle, "tblTitle", "TitleName", "TitlePK") & ", " &
                " [RegionFK]=" & PKComboBox(cmbRegion, "tblRegion", "RegionName", "RegionPK") & ", " &
    ...
    Private Sub btnDelete_Click(sender As System.Object, e As System.EventArgs) Handles btnDelete.Click
        ...
        Else
            ...
            If Result = DialogResult.Yes Then
                '// iRow is the CustomerPK or Primary key that is hidden.
                strStmt = " DELETE FROM tblCustomer WHERE CustomerPK = " & iRow
                '// UPDATE RECORD
                DoSQL(strStmt)
